...
const { extractAppName, extractDirectAppPath } = require("./app-utils");
...
function createMoveEngine(backend) {
  ...
  function getWindowIdentity(windowRef, index) {
    ...
    const appPath = extractDirectAppPath((windowRef && windowRef.application) || {});
  ...
  return {
    ...
    async moveAppWindowsToDirection(selectedAppPath, selectedAppName, direction) {
      ...
      const appWindows = allWindows.filter((windowRef) => matchesApp(windowRef, selectedAppPath, selectedAppName));
